...
5. Wait for the Pi to stretch and SCP the pred mask + timestamps back to `pred_masks/`
...
```
This automatically uses the timestamps file (`pred_mask_..._timestamps.csv`) if present for accurate frame alignment. A comparison image is saved to `tmp/mask_comparison.png` every run.
...
| File | Resolution | FPS |
|---|---|---|
| `GT_videos/gt_mask_1080p_30fps.mp4` | 1080×1920 | 30 |
| `GT_videos/gt_mask_720p_30fps.mp4` | 720×1280 | 30 |
| `GT_videos/gt_mask_480p_30fps.mp4` | 480×854 | 30 |

Source footage: `GT_videos/test_footage_40s.mp4` (3840×2160, portrait, 60fps original)
Test footage (sent to Pi): `GT_videos/test_footage_40s_1080p_30fps.mp4` etc.
...
```bat
venv\Scripts\activate
venv\Scripts\python generate_gt_sam2.py --input C:\Users\flemm\Documents\SAM-benchmark\test_footage.mp4 --output gt_mask.mp4 --debug
```
...
```bash
cd /home/flemming/Documents/GitHub/SAM-benchmark
venv/bin/python generate_gt_sam2.py --input /path/to/video.mp4 --output gt_mask.mp4 --debug
```
...
| File | Description |
|---|---|
| `gt_mask.mp4` | **The ground truth.** Binary mask video — white = person, black = background. This is what you compare the model against in `benchmark.py`. |
| `gt_mask_debug.mp4` | Visual verification video — original footage with green person overlay on the left, binary mask on the right. Use this to check the GT looks correct before running a full benchmark. Only created with `--debug`. |
| `gt_mask_frames/` | Extracted JPEG frames from the input video. Kept on disk so a resumed run skips re-extraction. Safe to delete after GT is confirmed correct — it just means re-extraction if you rerun. |
| `gt_mask_progress.pkl` | Checkpoint file for pause/resume. Deleted automatically when the run finishes successfully. If it exists, the next run will resume from where it left off. |
...
| File | Description |
|---|---|
| `pred_mask_{model}_{WxH}.mp4` | **The model prediction.** Binary mask video in the same format as `gt_mask.mp4`. Auto-named from the active model and resolution, auto-sent to `/home/flemming/Documents/GitHub/P6/pred_masks/` on this PC. |
...
```bat
python3 benchmark.py --gt gt_mask.mp4 --pred pred_mask.mp4 --log run_log.txt > results.txt
```
...
```bat
venv\Scripts\python generate_gt_sam2.py --input video.mp4 --output gt_mask.mp4 --conf 0.2
```
