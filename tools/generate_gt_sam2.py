...
def scan_all_frames(frames_dir, total_frames, conf, device, scan_debug_path=None):
    ...
    if scan_debug_path and scan_frames_with_boxes:
        ...
        writer = cv2.VideoWriter(str(scan_debug_path), fourcc, 5.0, (fw, fh))
        if not writer.isOpened():
            writer = cv2.VideoWriter(str(scan_debug_path), cv2.VideoWriter_fourcc(*"mp4v"), 5.0, (fw, fh))
...
def main():
    ...
    prompts_path = cache_root / "prompts.pkl"
    debug_path = run_root / "debug.mp4"
    scan_debug_path = (run_root / "scan_debug.mp4") if args.scan_debug else None
    ...
    with torch.inference_mode(), (torch.autocast(device, dtype=dtype) if device == "cuda" else contextlib.nullcontext()):
        inference_state = predictor.init_state(video_path=str(frames_dir), offload_video_to_cpu=True, offload_state_to_cpu=True)
        ...
        for frame_idx, obj_ids, mask_logits in tqdm(
            ...
            cv2.imwrite(str(masks_dir / f"{frame_idx:06d}.png"), combined)
