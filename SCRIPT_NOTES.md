...
Auto-generated at `tmp/mask_comparison.png` on every run.
...
### 7. Output folder
All outputs go to `<benchmark_repo>/tmp/` — never `/tmp`.
...
- [ ] Batch mode — accept CSV/config listing (gt_path, pred_path, options) pairs, output summary table
