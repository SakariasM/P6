...
# [PC] — run benchmark after data/preds/ is populated
...
"$FFMPEG" -stream_loop -1 -re -i data/gt/test_footage_40s_1080p_30fps.mp4 -f mpegts udp://192.168.10.3:1234
```
...
GT videos for all resolutions are pre-generated in `data/gt/`.
```bash
bash tools/stream.sh data/gt/test_footage_40s_480p_30fps.mp4
...
Pass `--output /tmp/debug_output.mp4` to live_mask.py inside run_benchmark.sh. The copied file lands in `runs/stream/<run-id>/debug_output.mp4`.
...
GT videos are already generated and stored in `data/gt/`. To regenerate:
```bat
...
bash tools/stream.sh data/gt/test_footage_40s_1080p_30fps.mp4
```
This kills leftover Pi processes, starts `run_benchmark.sh` on the Pi, streams the video, and waits for the Pi to SCP the pred mask and timestamps back to `data/preds/`.
...
python3 tools/benchmark.py \
  ...
  --pred data/preds/pred_mask_yolo26n-seg_1080x1920.mp4
```
Comparison image is auto-saved to `runs/benchmark/<run-id>/mask_comparison.png` every run.
...
- [x] Comparison image auto-generated at `runs/benchmark/<run-id>/mask_comparison.png` on every run
